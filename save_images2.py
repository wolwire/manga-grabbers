...
while(nav_next):
	...
	img = soup.findAll('picture')
	
	
	if not os.path.isdir(f"D:/made_in_abyss/{manga_name}"):
		os.mkdir(f"D:/made_in_abyss/{manga_name}")
	count = 1
	im_list = []
	for i in img:
		count = count+1
		...
		src = image.get('src')
		a = urlparse(src)
		urllib.request.urlretrieve(src, f"D:/made_in_abyss/{manga_name}/{os.path.basename(a.path)}")
		cover = Image.open(f"D:/made_in_abyss/{manga_name}/{os.path.basename(a.path)}")
		cover = cover.convert('RGB')
		im_list.append(cover)
	im_list[0].save(f'D:/made_in_abyss/pdf-mangas/{manga_name}.pdf',save_all=True, append_images=im_list[1:])
